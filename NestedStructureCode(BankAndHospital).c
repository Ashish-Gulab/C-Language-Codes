#include<stdio.h>    
#include<string.h>
struct patient_hospital_detail
{
  int patient_id;
...
struct patient_detail;
{
  int id;
  char name[20];
  struct patient_hospital_detail hos_data;
}stu_data;
int main()
{  
  struct patient_detail pat_data ={2,"Ashish","ABC hospital"};
  printf("Id is %d\n", pat_data.id);
  printf("Name is %d\n",pat_data.name);

  printf("Hospital Id is %d\n",pat_data.hos_data.hospital_id);
  printf("Hospital Name is %s\n",pat_data.hos_data.hospital_name);
  return 0;
}
/*#include <stdio.h>
